...
#key <- "AIzaSyAD1g6QxYLtpUpJvAGRLA00j9WcEzjJlj0"

#get_transit_dist <- function(origin_lat, origin_lng){
#  orig <- paste(toString(origin_lat), "+", toString(origin_lng), sep= "")
#  transits <- google_places(location = c(origin_lat,origin_lng),
#                            place_type = 'transit_station',
#                            radius = 5000,
#                            key = key)
#  lats <- transits$results$geometry$location$lat[1]
#  logs <- transits$results$geometry$location$lng[1]
#  transit <- paste(toString(lats), "+", toString(logs), sep="")
#  dist <- gmapsdistance(origin = orig, destination = transit, mode = "walking")
#  return(dist)
#}
...
if (!reprocess) {
  ...
  #  full.data[i, "dtransit"] <- get_transit_dist(full.data$longitude[i], full.data$latitude[i])
  #}
...
```{r}
# changiing data for map visualization
...
cat("MSE for Boston Data using Lasso", rmse(boston.dummied.model_selection, lasso.model.data), "$\n")
cat("MSE for Boston Downtown Data using Lasso", rmse(boston.ddowntown.dummied.model_selection, lasso.model.ddowntown), "$\n")
cat("MSE for Boston Airport Data using Lasso", rmse(boston.dairport.dummied.model_selection, lasso.model.dairport), "$\n")
cat("MSE for Boston Both Data using Lasso", rmse(boston.dboth.dummied.model_selection, lasso.model.dboth), "$\n")
```
...
cat("MSE for Boston Data using Ridge", rmse(boston.dummied.model_selection, ridge.model.data), "$\n")
cat("MSE for Boston Downtown Data using Ridge", rmse(boston.ddowntown.dummied.model_selection, ridge.model.ddowntown), "$\n")
cat("MSE for Boston Airport Data using Ridge", rmse(boston.dairport.dummied.model_selection, ridge.model.dairport), "$\n")
cat("MSE for Boston Both Data using Ridge", rmse(boston.dboth.dummied.model_selection, ridge.model.dboth), "$\n")
```

```{r}
cat("MSE for Boston Data using Lasso - Validation Set 1", rmse(boston.dummied.validation_1, lasso.model.data), "$\n")
cat("MSE for Boston Downtown Data using Lasso - Validation Set 1", rmse(boston.ddowntown.dummied.validation_1, lasso.model.ddowntown), "$\n")
cat("MSE for Boston Airport Data using Lasso - Validation Set 1", rmse(boston.dairport.dummied.validation_1, lasso.model.dairport), "$\n")
cat("MSE for Boston Both Data using Lasso - Validation Set 1", rmse(boston.dboth.dummied.validation_1, lasso.model.dboth), "$\n")
...
plot_predicts <-function(predicted_price, real_price, text, color){
  ...
  ylab = "predicted prices", main = text,xlim= c(0,500), ylim= c(0,500), col = color, asp=1)
  abline(a = 0, b = 1)
...
```{r}
gam.ddowntown <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + s(bathrooms, bs="cs") + s(bedrooms, k=5, bs="cs") + bed_type + s(guests_included, bs="cs") + s(minimum_nights, bs="cs") + s(number_of_reviews, bs="cs") + instant_bookable + is_business_travel_ready + cancellation_policy + s(ddowntown, bs="cs"), data=boston.ddowntown.training, select=T, method="REML")
...
gam.best.rmse <- rmse(boston.data.validation_1, gam.best)
gam.best.rmse # 55.43666
