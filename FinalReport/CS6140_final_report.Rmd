...
### Neighborhood
```{r}
gam.neighborhood <- gam(price ~ host_is_superhost + host_identity_verified + neighborhood + property_type + room_type + s(accommodates, bs="cs") + bathrooms + s(bedrooms, k=5, bs="cs") + bed_type + s(guests_included, bs="cs") + minimum_nights + number_of_reviews + instant_bookable + is_business_travel_ready + cancellation_policy, data=boston.data.training, select=T, method="REML")
summary(gam.neighborhood)
# R^2: 0.615

gam.neighborhood.rmse <- rmse(boston.data.model_selection, gam.neighborhood)
gam.neighborhood.rmse # 52.39049
```

### Distance to both airport and downtown
```{r}
gam.dboth <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + bathrooms + s(bedrooms, k=5, bs="cs") + bed_type + s(guests_included, bs="cs") + minimum_nights + number_of_reviews + instant_bookable + is_business_travel_ready + cancellation_policy + s(ddowntown, bs="cs") + s(dairport, bs="cs"), data=boston.dboth.training, select=T, method="REML")
summary(gam.dboth)
# R^2: 0.608

gam.dboth.rmspe <- rmse(boston.dboth.model_selection, gam.dboth)
gam.dboth.rmspe # 52.65998
```

### Distance to airport
```{r}
gam.dairport <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + bathrooms + s(bedrooms, k=5, bs="cs") + bed_type + s(guests_included, bs="cs") + minimum_nights + number_of_reviews + instant_bookable + is_business_travel_ready + cancellation_policy + s(dairport, bs="cs"), data=boston.dairport.training, select=T, method="REML")
summary(gam.dairport)
# R^2: 0.599

gam.dairport.rmse <- rmse(boston.dairport.model_selection, gam.dairport)
gam.dairport.rmse # 52.78772
```

### Distance to downtown
```{r}
gam.ddowntown <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + bathrooms + s(bedrooms, k=5, bs="cs") + bed_type + s(guests_included, bs="cs") + s(minimum_nights, bs="cs") + number_of_reviews + instant_bookable + is_business_travel_ready + cancellation_policy + s(ddowntown, bs="cs"), data=boston.ddowntown.train, select=T, method="REML")
summary(gam.ddowntown)
# R^2: 0.598

gam.ddowntown.rmse <- rmse(boston.ddowntown.model_selection, gam.ddowntown)
gam.ddowntown.rmse # 52.28805
```
