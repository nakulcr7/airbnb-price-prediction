...
# distance to airport
split <- data_split(boston.dairport)
...
split <- data_split(boston.ddowntown)
...
### Final GAMs with variable selection through REML Shrinkage

#### Original
```{r}
gam8 <- gam(price ~ host_is_superhost + host_identity_verified + neighborhood + property_type + room_type + s(accommodates, bs="cs") + s(bathrooms, bs="cs") + s(bedrooms, k=5, bs="cs") + s(beds, bs="cs") + bed_type + s(guests_included, bs="cs") + s(minimum_nights, bs="cs") + s(number_of_reviews, bs="cs") + instant_bookable + is_business_travel_ready + cancellation_policy, data=boston.train, select=T, method="REML")
summary(gam8)
# Deviance exlained = 61.8%

gam8.rmspe <- rmspe(boston.test, gam8)
gam8.rmspe # 55.5039
```

#### Distance to both airport and downtown
```{r}
gam6 <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + s(bathrooms, bs="cs") + s(bedrooms, k=5, bs="cs") + s(beds, bs="cs") + bed_type + s(guests_included, bs="cs") + s(minimum_nights, bs="cs") + s(number_of_reviews, bs="cs") + instant_bookable + is_business_travel_ready + cancellation_policy + s(ddowntown, bs="cs") + s(dairport, bs="cs"), data=boston.dboth.train, select=T, method="REML")
summary(gam6)
# Deviance exlained = 59.8%
...
#### Distance to airport
```{r}
gam9 <- gam(price ~ host_is_superhost + host_identity_verified + property_type + room_type + s(accommodates, bs="cs") + s(bathrooms, bs="cs") + s(bedrooms, k=5, bs="cs") + s(beds, bs="cs") + bed_type + s(guests_included, bs="cs") + s(minimum_nights, bs="cs") + s(number_of_reviews, bs="cs") + instant_bookable + is_business_travel_ready + cancellation_policy + s(dairport, bs="cs"), data=boston.dairport.train, select=T, method="REML")
summary(gam9)
# Deviance exlained = 60.7%

gam9.rmspe <- rmspe(boston.dairport.test, gam9)
gam9.rmspe # 54.00618
```
...
summary(gam10)
# Deviance exlained = 60.5%

gam10.rmspe <- rmspe(boston.ddowntown.test, gam10)
gam10.rmspe # 59.51033
```
